...
export function OrderTracking() {
    ...
    if (!order) {
        return (
            <div className="min-h-screen bg-app-cream flex flex-col items-center justify-center p-8 text-center">
                <h1 className="text-2xl font-bold text-app-green mb-2">Order Not Found</h1>
                <p className="text-app-text-light mb-6">The order you are looking for does not exist.</p>
                <button onClick={() => navigate("/orders")} className="px-6 py-2 bg-app-green text-white font-medium rounded-xl hover:bg-app-green-light transition-all">
                    Go to My Orders
                </button>
            </div>
        );
    ...
    return (
        <div className="min-h-screen mb-20 bg-app-cream">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {/* Header */}
                <button onClick={() => navigate('/orders')} className="flex items-center gap-2 text-sm text-app-text-light hover:text-app-green mb-6 transition-colors group">
                    <ArrowLeftIcon className="size-4 group-hover:-translate-x-0.5 transition-transform" /> Back to Orders
                </button>
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <span className={`px-4 py-1.5 text-sm font-semibold rounded-full ${statusColors[order.status] || 'bg-gray-100 text-gray-700'}`}>
                        {order.status}
                    </span>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {order?.deliveryPartner && order.status !== "Delivered" && order.status !== "Cancelled" && (
                            <div className="bg-white rounded-2xl flex p-5 items-center justify-between">
                        ...
                        <div className="bg-white rounded-2xl p-5">
                            ...
                            <div className="mt-4 pt-3 border-t border-app-border space-y-1.5 text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-semibold">{currency}{order?.subtotal.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-app-text-light">Delivery</span>
                                    <span className="font-semibold">{order?.deliveryFee === 0 ? 'Free' : `${currency}${order?.deliveryFee.toFixed(2)}`}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-app-text-light">Tax</span>
                                    <span className="font-semibold">{currency}{order?.tax.toFixed(2)}</span>
                                </div>
