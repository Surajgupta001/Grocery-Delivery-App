...
export function FilterPanel({ categories, category, minPrice, maxPrice, updateFilter, clearFilters, hasFilters }: FilterPanelProps) {
                ...
                <div className="space-y-1.5">
                    {categoriesWithAll.map((cat) => (
                        <button key={cat.slug}
                            ...
                            className={`block w-full text-left px-3 py-2 text-sm rounded-md transition-all ${category === cat.slug ? 'bg-app-green text-white' : 'hover:bg-app-cream text-app-text-light'}`}
                        >
                            {cat.name}
                        </button>
                    ))}
