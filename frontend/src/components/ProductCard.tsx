        ...
        <div className="bg-white rounded-2xl overflow-hidden shadow hover:shadow-md transition-all duration-300 group animate-fade-in cursor-pointer" onClick={() => navigate(`/products/${product._id}`)}>
            ...
            <div className="relative aspect-square overflow-hidden">
                ...
                <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
                    {product.discount > 0 && (
                        <span className='px-2 py0.5 text-[10px] font-semibold uppercase bg-app-orange text-white rounded-full'>
            ...
            <div className="p-3.5 text-zinc-700">
                <h3 className="flex items-center gap-1 mb-2">{product.name}</h3>
                {/* Rating */}
                {product.rating > 0 && (
                ...
                <div className='flex items-center justify-between'>
                    ...
                    <button onClick={(e) => { e.stopPropagation(); addToCart(product) }} className='size-7 rounded-full bg-app-orange text-white flex-center hover:bg-app-orange-dark transition-colors shrink-0 active:scale-95'>
