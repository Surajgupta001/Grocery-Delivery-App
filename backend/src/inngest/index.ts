...
const sendMonthlyOffers = inngest.createFunction(
    {
        ...
        triggers: [{
            cron: "0 10 1 * *"
        }], // At 10:00 AM on the 1st day of every month
    },
    async ({ step }) => {
        // Fetch all products with low stock for the monthly offers email
        ...
        // Send in bactches of 10 to avoid overwhelming the email service
        const batchSize = 10;
...
const autoAssignRider = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        // Wait for 5 minutes before attempting to auto-assign a rider
        ...
        const result = await step.run(
            ...
            async () => {
                ...
                // Skip if order doesn't exist or already has a rider assigned
                if (!order || order.deliveryPartnerId) {
                    return {
                        skipped: true,
                        reason: !order ? 'Order not found' : 'Rider already assigned',
                    }
                }
                ...
                // Find an available delivery partner (rider)
                ...
                if (!availableRider) {
                    return {
                        skipped: true,
                        reason: 'No available delivery partners',
                    }
                }
                ...
                history.push({
                    ...
                    note: `Rider ${availableRider.name} assigned with OTP ${otp}`,
                })
